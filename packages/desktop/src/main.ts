...
async function ensureProxy(): Promise<boolean> {
  ...
  for (let port = PROXY_PORT_MIN; port <= PROXY_PORT_MAX; port++) {
    ...
    dialog.showErrorBox(
      "Veyr",
      `The Veyr proxy did not start within ${PROXY_START_TIMEOUT_MS / 1000} seconds on port ${port}.\n\nCheck the logs and reopen the app.`
    );
    return false;
  }
  ...
  dialog.showErrorBox(
    "Veyr",
    `No free port found in ${PROXY_PORT_MIN}-${PROXY_PORT_MAX} for the Veyr proxy.`
  );
  return false;
}
...
async function updateTray(): Promise<void> {
  ...
  if (overview?.today) {
    ...
    tray.setToolTip(
      `Veyr — today ${fmtUsd(overview.today.cost)} · month ${fmtUsd(overview.month?.cost)}`
    );
  } else {
    tray.setTitle(" Veyr");
    tray.setToolTip("Veyr");
  ...
  try {
    ...
    autoUpdater.on("update-available", () => {
      try {
        new Notification({
          title: "Veyr",
          body: "A Veyr update is available and will install on next launch.",
        }).show();
      } catch (err) {
