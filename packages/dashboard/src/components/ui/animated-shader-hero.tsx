...
export function Hero({ trustBadge, headline, subtitle, buttons, className = "" }: HeroProps) {
  ...
  return (
    <div className={`relative w-full h-screen overflow-hidden bg-black ${className}`}>
      ...
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-white">
        {/* Trust Badge */}
        {trustBadge && (
          <div className="mb-8 shader-hero-fade-in-down">
            <div className="flex items-center gap-2 px-6 py-3 bg-[#076EFF]/10 backdrop-blur-md border border-[#4FABFF]/30 rounded-full text-sm">
              {trustBadge.icons && (
                <div className="flex">
                  {trustBadge.icons.map((icon, index) => (
                    <span key={index} className="text-[#B1C5FF]">
                      {icon}
                    </span>
                  ))}
                </div>
              )}
              <span className="text-[#B1C5FF]">{trustBadge.text}</span>
            </div>
          </div>
        )}
        ...
        <div className="text-center space-y-6 max-w-5xl mx-auto px-4">
          ...
          <div className="max-w-3xl mx-auto shader-hero-fade-in-up shader-hero-delay-600">
          ...
          {buttons && (
            <div className="flex flex-col sm:flex-row gap-4 justify-center mt-10 shader-hero-fade-in-up shader-hero-delay-800">
              {buttons.primary && (
                <button
                  onClick={buttons.primary.onClick}
                  className="px-8 py-4 bg-gradient-to-r from-[#076EFF] to-[#4FABFF] hover:from-[#0B5FE0] hover:to-[#3D97F0] text-white rounded-full font-semibold text-lg transition-all duration-300 hover:scale-105 hover:shadow-xl hover:shadow-[#076EFF]/25"
                >
              ...
              {buttons.secondary && (
                <button
                  ...
                  className="px-8 py-4 bg-[#076EFF]/10 hover:bg-[#076EFF]/20 border border-[#4FABFF]/30 hover:border-[#4FABFF]/50 text-[#B1C5FF] rounded-full font-semibold text-lg transition-all duration-300 hover:scale-105 backdrop-blur-sm"
                >
...
void main(void) {
    ...
    for (float i=1.; i<12.; i++) {
        ...
        col=mix(col,vec3(bg*.05,bg*.14,bg*.30),d);
    }
    // Retint the multicolor filament streaks toward Veyr's blue accent (#076EFF
    // family); the cloud term above is already blue-dominant at the source.
    col*=vec3(0.55,0.75,1.25);
    O=vec4(col,1);
}`;
