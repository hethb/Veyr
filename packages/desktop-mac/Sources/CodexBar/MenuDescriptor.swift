...
struct MenuDescriptor {
    ...
    enum MenuActionSystemImage: String {
        ...
        case veyrSpend = "dollarsign.circle"
    }
    ...
    enum MenuAction: Equatable {
        ...
        case veyrSpend(filterTag: String?)
    }
    ...
    {
        var sections: [Section] = [Self.veyrSpendSection()]
    ...
    /// Veyr: persistent spend header (top of every menu) + dashboard entry.
    private static func veyrSpendSection() -> Section {
        let spend = VeyrSpend.shared
        let today = spend.todaySpend
        let week = spend.thisWeekSpend
        var header = "Today: \(VeyrFormat.usd(today.costUSD))  ·  " +
            "This week: \(VeyrFormat.usd(week.costUSD))"
        var topTag: String?
        if let top = spend.monthlyTagSpend.first {
...
extension MenuDescriptor.MenuAction {
    var systemImageName: String? {
        ...
        case .veyrSpend: MenuDescriptor.MenuActionSystemImage.veyrSpend.rawValue
        }
    }
}
