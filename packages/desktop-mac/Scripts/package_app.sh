...
patch_keyboard_shortcuts
# Command Line Tools lack the PreviewsMacros plugin; strip #Preview from checkouts.
python3 "$ROOT/Scripts/strip_preview_macros.py"
...
APP_FINAL="$ROOT/Veyr.app"
APP_STAGE="$ROOT/.build/package/Veyr.app"
rm -rf "$APP_STAGE"
APP="$APP_STAGE"
...
BUNDLE_ID="com.veyr.mac"
FEED_URL=""
AUTO_CHECKS=false
if [[ "$LOWER_CONF" == "debug" ]]; then
  BUNDLE_ID="com.veyr.mac.debug"
  FEED_URL=""
  AUTO_CHECKS=false
fi
...
APP_GROUP_ID="${APP_TEAM_ID}.com.veyr.mac"
if [[ "$BUNDLE_ID" == *".debug"* ]]; then
  APP_GROUP_ID="${APP_TEAM_ID}.com.veyr.mac.debug"
fi
...
<plist version="1.0">
<dict>
    <key>CFBundleName</key><string>Veyr</string>
    <key>CFBundleDisplayName</key><string>Veyr</string>
    ...
    <key>NSHumanReadableCopyright</key><string>© 2026 Veyr. Built on CodexBar © Peter Steinberger. MIT License.</string>
...
# The widget extension needs full Xcode (xcodebuild); skip it on CLT-only machines.
if xcodebuild -version >/dev/null 2>&1 && [[ "${VEYR_SKIP_WIDGET:-0}" != "1" ]]; then
  install_widget_extension
  strip_release_binary "$APP/Contents/PlugIns/CodexBarWidget.appex/Contents/MacOS/CodexBarWidget"
else
  echo "WARN: Skipping widget extension (full Xcode not available or VEYR_SKIP_WIDGET=1)." >&2
fi
