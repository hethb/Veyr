# PromptLens for VSCode

Brings PromptLens cost attribution and optimization suggestions into your
editor, and routes **Claude Code** through the PromptLens proxy so its usage is
logged like any other LLM traffic.

## Features

- **PromptLens panel** in the Activity Bar — shows today / week / month spend
...
3. Click the PromptLens icon in the Activity Bar.
...
## Routing Claude Code through PromptLens
...
2. Run the command **PromptLens: Route Claude Code through proxy** (Command
...
3. Open a **new** terminal and run `claude` as usual. Its requests now appear in
   the PromptLens panel and dashboard.

Run **PromptLens: Stop routing Claude Code through proxy** to undo it.
